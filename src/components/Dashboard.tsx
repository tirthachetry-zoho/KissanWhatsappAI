      ...
      <div className="max-w-[1400px] mx-auto px-5 lg:px-6" ref={ref}>
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-semibold mb-6 text-[var(--color-midnight)]">
            Real-Time <span className="text-[var(--color-primary)]">Farm Intelligence</span>
          </h2>
          <p className="text-[var(--color-muted-foreground)] text-lg max-w-xl mx-auto">
            Aggregated data from thousands of farmer queries — powering predictive insights for agri-input companies and insurers.
          </p>
        </motion.div>
        ...
        >
          {[
            { value: dashboardData.stats.totalQueries, label: 'Total Queries', suffix: '+', icon: '📊' },
            { value: dashboardData.stats.farmersServed, label: 'Farmers Served', suffix: '+', icon: '👨‍🌾' },
            { value: dashboardData.stats.languagesSupported, label: 'Languages', suffix: '', icon: '🌐' },
            { value: dashboardData.stats.statesCovered, label: 'States', suffix: '', icon: '🗺️' },
          ].map((stat, i) => (
          ...
          <div className="glass-card rounded-xl p-4 text-center">
            ...
            <div className="text-xs text-[var(--color-muted-foreground)] mt-1">Avg Response</div>
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <div className="text-xl mb-2">🎯</div>
            <div className="text-2xl md:text-3xl font-bold text-[var(--color-primary)]">{dashboardData.stats.accuracy}</div>
            <div className="text-xs text-[var(--color-muted-foreground)] mt-1">Accuracy</div>
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-6">Top Query Categories</h3>
              ...
              {dashboardData.topQueries.map((query, i) => (
                <div key={i}>
                  ...
                  <div className="h-2.5 bg-[var(--color-muted)] rounded-full overflow-hidden">
                    <div
                      className="bar-fill h-full rounded-full bg-gradient-to-r from-[var(--color-primary)] to-emerald-400"
                      style={{
                        width: isInView ? `${(query.count / maxCount) * 100}%` : '0%',
          ...
          <motion.div
            ...
            className="glass-card rounded-2xl p-6"
          >
            <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-6">Regional Activity</h3>
            <div className="grid grid-cols-2 gap-3">
              {dashboardData.regions.map((region, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="rounded-xl p-3 border border-[var(--color-border)] transition-all hover:border-[var(--color-primary)]/30 hover:shadow-sm"
                    style={{ background: `rgba(36, 143, 71, ${intensity * 0.08})` }}
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {region.crops.map((crop, ci) => (
                        <span key={ci} className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--color-muted)] text-[var(--color-muted-foreground)]">
                          {crop}
                        </span>
                      ))}
                    </div>
