      ...
      <div className="max-w-[1400px] mx-auto px-5 lg:px-6" ref={ref}>
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-semibold mb-6 text-[var(--color-midnight)]">
            How KissanAI <span className="text-[var(--color-primary)]">Works</span>
          </h2>
          <p className="text-[var(--color-muted-foreground)] text-lg max-w-xl mx-auto">
            From voice note to actionable advice — in under 3 seconds. Here&apos;s the magic under the hood.
          </p>
        </motion.div>
        ...
        <div className="hidden lg:block mb-16">
          <div className="flex items-center justify-between relative">
            ...
            {pipelineSteps.map((step, i) => (
              ...
              >
                <div
                  className={`w-28 h-28 rounded-2xl flex items-center justify-center text-3xl transition-all duration-500 ${
                    i <= activeStep
                      ? 'bg-[var(--color-primary)]/10 border-2 border-[var(--color-primary)]/30 scale-110 shadow-lg shadow-[var(--color-primary)]/10'
                ...
                <div className="mt-4 text-center">
                  <div
                    className={`text-xs font-bold uppercase tracking-wider mb-1 transition-colors ${
                      i <= activeStep ? 'text-[var(--color-primary)]' : 'text-[var(--color-muted-foreground)]'
                    }`}
                  >
                    Step {step.step}
                  </div>
          ...
          {pipelineSteps.map((step, i) => (
            <motion.div
              ...
              className={`flex items-start gap-4 p-4 rounded-2xl transition-all duration-500 ${
                i <= activeStep ? 'bg-white shadow-md border border-[var(--color-primary)]/20' : 'bg-white/50'
              }`}
            >
              <div
        ...
        <motion.div
          ...
          className="glass-card rounded-2xl p-6 max-w-2xl mx-auto text-center"
        >
